        ...
        else if(OVR_stristr(commandName, "PauseUntil") == commandName)
        {
            ...
            if(argCount >= 2)
            {
                ...
                if(argCount == 3) {
                    // wait timeout
                    if(OVR_stristr(waitUnits, "ms") == waitUnits) {
                        p->WaitUnits = CommandPauseUntil::ms;
                    }
                    else if(OVR_stristr(waitUnits, "s") == waitUnits) {
                        p->WaitUnits = CommandPauseUntil::s;
                    }
                    else if(OVR_stristr(waitUnits, "frame") == waitUnits) {
                        // Covers both "frame" and "frames".
                        p->WaitUnits = CommandPauseUntil::frames;
                    }
                    else {
                ...
                WriteLog(
                    "[OWDScript] Invalid PauseUntil command: %s. Expect PauseUntil <mode> <amount> [ms|s|frame|frames]\n",
                    line.c_str());
                success = false;
            }
        }
        ...
        if (CurrentCommand < Commands.size())
        {
            ...
            switch (command.commandType)
            {
                ...
                case CommandType::PauseUntil: {
                    ...
                    {
                        if (c.WaitUnits == CommandPauseUntil::ms) {
                    ...
                    // check wait mode
                    if ((OWDApp->HmdStatus & ovrStatus_PositionTracked) && (c.WaitMode == CommandPauseUntil::dof6))
                    {
                        WriteLog("[OWDScript] PauseUntil completed on %s\n", waitModeStr.c_str());
                        CurrentCommand++; // Move onto the next instruction.
                    }
                    else if (!(OWDApp->HmdStatus & ovrStatus_PositionTracked) && (c.WaitMode == CommandPauseUntil::dof3))
                    {
                          WriteLog("[OWDScript] PauseUntil completed on %s\n", waitModeStr.c_str());
                          CurrentCommand++; // Move onto the next instruction.
                    }
                    else if ((c.WaitUnits == CommandPauseUntil::ms) ||
                        (c.WaitUnits == CommandPauseUntil::s)) // check wait timeout
                    {
                        if (ovr_GetTimeInSeconds() >= c.CompletionValue) // If timeout
                        {
                            WriteLog(
                                ...
                                waitModeStr.c_str(),
                                c.WaitAmount,
                                (c.WaitUnits == CommandPauseUntil::ms) ? "ms" : "s");
                            CurrentCommand++; // Move onto the next instruction.
                         }
                    }
                    else if (c.WaitUnits == CommandPauseUntil::frames)
                    {
